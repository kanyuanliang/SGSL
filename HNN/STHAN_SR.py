...
class STHAN_SRlayer(nn.Module):
	def __init__(self,input_dimension,output_dimension):
		super(STHAN_SRlayer,self).__init__()
	...
	def forward(self,x,H_shape):
		...
		hyperedge_feature=hyperedge_feature/torch.unsqueeze(torch.sum(H_shape.transpose(-2,-1),dim=-1),dim=-1)
...
class GNNlayer(nn.Module):
	def __init__(self,input_dimension,output_dimension,num_heads=4):
		...
		self.attention_module = nn.ModuleList([STHAN_SRlayer(input_dimension,output_dimension) for _ in range(num_heads)])
